...
/*   Updated: 2020/11/08 13:46:55 by gmarva           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *ft_get_texture_color(t_all *all, int h, int w, int textmum)
{
    ...
    color = all->text[textmum].addr + ((int)h * all->text[textmum].line_length + (int)w * (all->text[textmum].bits_per_pixel/ 8));
    return (color);
}
...
int ft_get_wall_color(t_all *all, t_wall wall, int i)
{
    ...
    while (wall.start < wall.end)
    {
		if (wall.start >= 0)
		{
            color = ft_get_texture_color(all, (h * all->text[textnum].img_height) / wall.height, w, textnum);
